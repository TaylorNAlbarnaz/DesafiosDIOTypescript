...
searchButton?.addEventListener('click', async () => {
  ...
  let listaDeFilmes: IMovieList = await procurarFilme(query);
...
class HttpClient {
  static async get({url, method, body = null}: IRequestHTTP) {
...
async function procurarFilme(query: string){
  ...
  let result: IMovieList = await HttpClient.get({
...
async function criarSessao() {
  let result: ISession = await HttpClient.get({
